...
if __name__ == '__main__':
    ...
    parser.add_argument('--protein_pdb', type=str, default=None,
                    help='Absolute path to receptor PDB to use for docking when ligand_filename is missing')
    ...
    results_fn_list = glob(os.path.join(args.sample_path, 'sample.pt'))
    # results_fn_list = sorted(results_fn_list, key=lambda x: int(os.path.basename(x)[:-3].split('_')[-1]))
    results_fn_list = sorted(results_fn_list, key=lambda x: 0)
        ...
        for sample_idx, (pred_pos, pred_v) in enumerate(zip(all_pred_ligand_pos, all_pred_ligand_v)):
            ...
            try:
                ...
                if args.docking_mode == 'qvina':
                    ...
                    lig_fn = getattr(r['data'], 'ligand_filename', None)
                    if args.protein_pdb is not None:
                        # Use a direct constructor if available; otherwise add a small helper in VinaDockingTask
                        vina_task = QVinaDockingTask(args.protein_pdb, mol)
                    else:
                        vina_task = QVinaDockingTask.from_generated_mol(mol, lig_fn, protein_root=args.protein_root)
                    ...
                    if args.protein_pdb is not None:
                        # Use a direct constructor if available; otherwise add a small helper in VinaDockingTask
                        vina_task = VinaDockingTask(args.protein_pdb, mol)
                    else:
                        vina_task = VinaDockingTask.from_generated_mol(mol, lig_fn, protein_root=args.protein_root)
            ...
            results.append({
                'mol': mol,
                'smiles': smiles,
                'ligand_filename': getattr(r['data'], 'ligand_filename', 'custom.pdb'),
